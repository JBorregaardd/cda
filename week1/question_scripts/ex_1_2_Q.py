...
for l in lambdas:
    ...
    for _ in range(n_simulations):
        ...
        ridge_fit = Ridge(alpha=l)
        ridge_fit.fit(X_fixed, y)
        pred = ridge_fit.predict(x_eval)
        preds.append(pred[0])
    preds = np.array(preds)
    bias_sq.append((np.mean(preds) - target_val)**2)
    variance.append(np.var(preds))
...
# plt.show()
plt.figure(figsize=(10, 6))
plt.plot(lambdas, bias_sq, label='Bias^2', marker='o')
...
plt.xscale('log')
plt.gca().invert_xaxis()
plt.xlabel('Ridge Lambda (Complexity)')
plt.ylabel('Error')
